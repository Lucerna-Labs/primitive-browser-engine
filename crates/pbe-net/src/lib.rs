...
/// `body` is a UTF-8 string via lossy decoding; for content that may be binary
/// (images, other non-text responses), use [`fetch_bytes`] + [`FetchedBytes`]
/// instead so the raw byte stream survives without U+FFFD replacements
/// corrupting non-UTF-8 sequences.
#[derive(Clone, Debug)]
...
/// primitive that binary content (PNG/JPEG/BMP/font/binary blob) needs. HTML
...
/// Fetch a URL by driving the sealed system `curl` and return the response
/// as UTF-8-decoded text. HTML and CSS callers want this shape. For content
/// that may be binary (images, fonts, blobs), use [`fetch_bytes`] instead —
/// String-decoding non-UTF-8 bytes replaces them with U+FFFD, which corrupts
/// PNG/JPEG/BMP streams.
///
...
/// Fetch a URL by driving the sealed system `curl` and return the response
/// as raw bytes. This is the primitive image decoders + binary consumers
/// need — no UTF-8 lossy round-trip corrupts the stream. Same security
/// posture as [`fetch`] (http/https-only, curl-driven, no linked crypto).
...
/// pbe-net's zero-dep posture. Not the fastest possible (a Boyer-Moore
